// src/pages/customer-data/address-type/components/InlineAddressTypeForm.jsx
import React, { useState, useEffect } from 'react';
import ConfirmationModal from '../../../../components/common/ConfirmationModal';
import { X, AlertCircle, MapPin } from 'lucide-react';

const InlineAddressTypeForm = ({
  isVisible,
  onClose,
  onSubmit,
  initialData = null,
  ...
  useEffect(() => {
    if (isVisible) {
      setAddressTypeName(initialData?.address_type_name || '');
      setValidationError('');
    }
  ...
  const handleSubmit = (e) => {
    ...
    if (!addressTypeName.trim()) {
      setValidationError('Address type name is required');
      return;
    }
    
    // Prepare form data but don't submit yet
    const data = {
      address_type_name: addressTypeName.trim()
    };
    
    if (initialData?.address_type_id) {
      data.address_type_id = initialData.address_type_id;
    }
    ...
    <>
      <div className="bg-gradient-to-r from-primary-50 to-indigo-50 dark:from-gray-800 dark:to-gray-900 rounded-xl shadow-md p-8 mb-8 border border-primary-100 dark:border-gray-700 relative overflow-hidden min-h-[400px]">
        ...
        <div className="absolute right-0 top-1/2 transform -translate-y-1/2 mr-6 pointer-events-none opacity-10">
          <MapPin size={200} className="text-primary-600 dark:text-primary-400" />
        </div>
        ...
        <div className="flex justify-between items-center mb-6 pb-3 border-b border-primary-200 dark:border-gray-700 relative z-10">
          <div className="flex items-center">
            ...
            <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-200">
              {initialData ? 'Edit Address Type' : 'Add New Address Type'}
            </h2>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-8 relative z-10 py-4">
          <div className="mb-4">
            <label className="block text-gray-700 dark:text-gray-300 text-sm font-medium mb-3">
              Address Type Name <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              ...
              placeholder="Enter address type name (e.g., Permanent, Current, Billing)"
              disabled={isLoading}
              maxLength={100}
              required
            />
      ...
      <ConfirmationModal
        ...
        message={initialData 
          ? `Are you sure you want to update the address type "${addressTypeName}"?` 
          : `Are you sure you want to add a new address type "${addressTypeName}"?`}
...
export default InlineAddressTypeForm;